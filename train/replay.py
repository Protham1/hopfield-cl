...
def train_one_epoch_strict_replay(
    model,
    current_loader,
    buffer,
    optimizer,
    ...
    for x_cur, y_cur in current_loader:
        ...
        if buffer.n_tasks_stored > 0:
            all_samples = []
            for reservoir in buffer._storage.values():
            ...
            if len(all_samples) > 0:
                ...
                x_rep, y_rep = buffer.collate(samples)
                x_rep = x_rep.to(device)
                y_rep = y_rep.to(device)

                x = torch.cat([x_cur, x_rep], dim=0)
...
def run_replay(
    ...
    buffer = ReplayBuffer(capacity_per_task=capacity_per_task)

    if verbose:
        print(f"\nReplay baseline — {len(train_tasks)} tasks, {n_epochs} epochs each")
        print(f"Buffer: {capacity_per_task} samples/task")
    ...
    for task_id in range(len(train_tasks)):
        ...
        for epoch in range(n_epochs):
            loss = train_one_epoch_strict_replay(
                model,
                current_loader,
                buffer,
                optimizer,
        ...
        buffer.add_task(task_id, train_tasks[task_id])
